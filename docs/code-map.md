...
- Primary Google flow: click “使用 Google 登录 ChatGPT” -> create or reuse the app-owned `userData/chatgpt-login-browser` Chrome/Edge profile and a localhost-only DevTools port -> open `https://chatgpt.com/auth/login` directly -> complete the normal ChatGPT Google login in that real browser -> wait for a stable ChatGPT Auth.js session cookie -> read only exact `chatgpt.com` cookies through CDP -> copy them into `persist:speaksub-chatgpt` -> close the login browser while retaining its dedicated profile -> reload the real `chatgpt.com` page inside Electron -> `ChatGPTAutomation.isReady()` verifies the composer. Future reauthentication reuses the saved Google account without touching the user's normal Chrome profile.
