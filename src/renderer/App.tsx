...
import type { AutomationStatus, ConnectionState, CorrectionStrength, DictionaryResult, PracticeLifecycle, PracticeMode, PracticeSource, ProviderSettings, ReviewResult, SavedStudyItem, SessionArchiveItem, SubtitlePreferences, TranscriptEvent } from '../shared/types'
...
export function App() {
  ...
  const [connection, setConnection] = useState<ConnectionState>({ ready: false, pageVisible: true, activeProvider: 'chatgpt-web', providers: { 'chatgpt-web': false, 'gemini-web': false } })
  ...
  useEffect(() => {
    void Promise.all([window.speaksub.getState(), window.speaksub.listStudyItems(), window.speaksub.getProviderSettings(), window.speaksub.listSessions()]).then(([state, saved, provider, archived]) => {
      setSettings(state.settings); setConnection(state.connection); setAutomation(state.automation); setSession(state.session?.id); setEvents(state.events); setItems(saved); setProviders(provider); setSource(state.source); setMode(state.mode); setLifecycle(state.lifecycle); setSessions(archived)
    })
  ...
  async function chooseConnectionSource(next: 'chatgpt-web' | 'gemini-web'): Promise<void> { setSource(next); setConnection(await window.speaksub.showConnectionPage(next)) }
  ...
  async function clearPendingCleanup(): Promise<void> { if (source === 'api-direct') return; await window.speaksub.clearPendingCleanup(source); setAutomation({ phase: 'idle', message: '已清除上一条练习记录；现在可以重新开始。' }) }
  ...
  async function endPractice(): Promise<void> { if (lifecycle === 'ending') return; setLifecycle('ending'); capture.current.stop(); player.current.stop(); await window.speaksub.stopVoiceCapture(); try { const result = await window.speaksub.endPractice(); setSession(undefined); setReview(result.review); setLifecycle('idle'); setSessions(await window.speaksub.listSessions()); if (result.error) setAutomation({ phase: 'failed', message: result.error, recoverable: true }); else if (result.voiceWarning) setAutomation({ phase: 'failed', message: result.voiceWarning, recoverable: true }) } catch (error) { setLifecycle('error'); setAutomation({ phase: 'failed', message: error instanceof Error ? error.message : '无法结束练习。', recoverable: true }) } }
  ...
  async function saveLookup(): Promise<void> { const saved = await window.speaksub.saveStudyItem({ kind: selection.includes(' ') ? 'sentence' : 'word', sourceText: selection, note: lookup?.naturalAlternative }); setItems((current) => mergeSavedStudyItem(current, saved)) }
  async function deleteStudyItem(id: string): Promise<void> { await window.speaksub.deleteStudyItem(id); setItems((current) => current.filter((item) => item.id !== id)) }
  ...
  if (connection.pageVisible) return <main className="connection-shell"><section className="connection-panel">
    ...
    <div className="source-picker"><button className={source === 'chatgpt-web' ? 'active' : ''} onClick={() => void chooseConnectionSource('chatgpt-web')}>ChatGPT</button><button className={source === 'gemini-web' ? 'active' : ''} onClick={() => void chooseConnectionSource('gemini-web')}>Gemini</button></div><div className="connection-steps"><span>01 登录网页模型</span><span>02 确认账号状态</span><span>03 进入练习台</span></div>
  ...
  <section className="studio-content">
    ...
    {tab === 'library' && <section className="utility-page"><p className="kicker">LOCAL MARKDOWN ARCHIVE</p><h1>学习本</h1><div className="lookup-bar"><textarea value={selection} onChange={(event) => setSelection(event.target.value)} placeholder="输入想复习的单词、短语或句子" rows={2}/><button className="primary-action" onClick={() => void querySelection()}>查询解释</button></div>{lookup && <article className="lookup-result"><h2>{lookup.query}</h2><p>{lookup.definitions.join('；') || lookup.contextualMeaning}</p>{lookup.naturalAlternative && <p>更自然：{lookup.naturalAlternative}</p>}<button className="quiet-action" onClick={() => void saveLookup()}>收藏</button></article>}<div className="study-stream">{items.length ? items.map((item) => <article key={item.id}><span>{item.kind === 'word' ? 'WORD' : 'SENTENCE'}</span><strong>{item.sourceText}</strong><p>{item.note}</p><button className="quiet-action" onClick={() => void deleteStudyItem(item.id)}>删除</button></article>) : <p className="empty-copy">你的收藏会以 Markdown 文档保存在本机。</p>}</div><div className="session-history"><h2>练习历史</h2>{sessions.length ? sessions.map((item) => <article key={item.session.id}><small>{new Date(item.session.startedAt).toLocaleString()}</small><strong>{item.review?.topic ?? '口语练习'}</strong><p>{item.review?.summary ?? `${item.transcript.length} 条对话记录`}</p>{item.review?.issues.map((issue, index) => <div className="review-issue" key={index}><span>{issue.original}</span><strong>{issue.improved}</strong><small>{issue.reason}</small></div>)}{item.review?.nextPractice && <p>下一次：{item.review.nextPractice}</p>}</article>) : <p className="empty-copy">完成练习后会在这里显示复盘。</p>}</div></section>}
